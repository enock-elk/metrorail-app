// --- METRORAIL NEXT TRAIN LOGIC (V7_07.21 - Performance Polish Edition v1) ---
...
async function loadAllSchedules(force = false) {
    ...
    try {
        ...
        // when we have NO usable data at all. A post-commit failure (e.g. IndexedDB write throwing
        // after fullDatabase was already hydrated in RAM) must NOT trigger the false weak-signal modal,
        // because that also kicks off the map auto-return -> welcome-screen loop.
        const hasUsableData = (typeof fullDatabase !== 'undefined' && fullDatabase);
        if (!usedCache && !hasUsableData) {
